...
import { Menu, X, LogIn, LogOut, User, Loader2, ChevronDown, Trophy, UserCircle2 } from 'lucide-react';
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-400 ${
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between">
        ...
        {/* Desktop: Portfolio + Live Counter - Clickable */}
        <div className="hidden md:flex items-center gap-3 relative">
          {user && (
            <button
              onClick={() => handleNav('profile')}
              title="Your portfolio"
              className={`flex items-center gap-1.5 px-3 py-2 border rounded-sm transition-all ${
                currentPage === 'profile'
                  ? 'border-blue-500/60 bg-blue-950/30 text-white'
                  : 'border-blue-900/30 text-white/50 hover:text-white hover:border-blue-500/40'
              }`}
            >
              <Trophy className="w-3.5 h-3.5" />
              <span className="font-cyber text-xs tracking-widest">PORTFOLIO</span>
            </button>
          )}
                ...
                {user ? (
                  <div className="space-y-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        <div className="font-cyber text-xs text-white tracking-wide truncate">{user.displayName || 'User'}</div>
                        <div className="text-white/40 text-xs font-inter truncate">{user.email}</div>
                      </div>
                    </div>
                    <button
                      onClick={() => handleNav('profile')}
                      className="w-full flex items-center justify-center gap-2 px-3 py-2 border border-blue-500/30 bg-blue-950/20 hover:bg-blue-950/30 transition-colors text-xs font-inter text-white/80"
                    >
                      <UserCircle2 className="w-4 h-4" /> View portfolio
                    </button>
                    <button
                  ...
                  <div className="space-y-3">
                    <p className="text-white/50 text-xs font-inter leading-relaxed">
                      Sign in with Google to show up in the live counter and build your portfolio.
                    </p>
                    <button
          ...
          >
            <div className="px-4 py-4 flex flex-col gap-1">
              ...
              {user && (
                <button
                  onClick={() => handleNav('profile')}
                  className={`w-full text-left py-3 px-4 font-cyber text-sm tracking-widest transition-colors flex items-center gap-2 ${
                    currentPage === 'profile'
                      ? 'text-white bg-blue-600/10 border-l-2 border-blue-500'
                      : 'text-white/50 hover:text-white hover:bg-white/5'
                  }`}
                >
                  <Trophy className="w-4 h-4" /> PORTFOLIO
                </button>
              )}
